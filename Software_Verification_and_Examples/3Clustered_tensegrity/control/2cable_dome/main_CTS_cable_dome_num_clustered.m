...
n_c=[1 2 3 4 6 12];   % use n_c number of clustered string in one group
for i=1:6
num_clu=n_c(i);
...
[U1_2,U2_2,V1_2,V2_2,S1_2]=tenseg_svd(A_1ag_2);  % in clustered string
...
plot_mode(K_mode,k,N,Ia,C_b,C_s,l,'tangent stiffness matrix',...
    'Order of Eigenvalue','Eigenvalue of Stiffness (N/m)',num_plt,1,saveimg,[0,50]);
end
...
%% record the result
n_t=[n_t,N(:)];
num_prestress_mode(i)=size(V2_2,2);
...
figure
plot(n_c,num_prestress_mode,'-o','linewidth',2);
set(gca,'fontsize',18);grid on;
xlabel('{\itn_e}','fontsize',18);   %number of cluster strings in one gruop
ylabel('{\itn_p}','fontsize',18);   %number of prestress mode 
figure
plot(n_c,minimal_stiffness,'-o','linewidth',2);
set(gca,'fontsize',18); grid on;
xlabel('{\itn_e}','fontsize',18);
ylabel('\lambda_{min}','fontsize',18); %minimal stiffness eigenvalue
figure
plot(n_c,minimal_frequency,'-o','linewidth',2);
set(gca,'fontsize',18); grid on;
xlabel('{\itn_e}','fontsize',18);
