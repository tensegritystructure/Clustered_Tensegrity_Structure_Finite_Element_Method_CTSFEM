...
h=0.10*2*R;   %hight of the dome
beta=20*pi/180*ones(m,1);    %all angle of diagonal string
...
pinned_X=([3:3:3*p])'; pinned_Y=([3:3:3*p])'; pinned_Z=([3:3:3*p])';
[Ia,Ib,a,b]=tenseg_boundary(pinned_X,pinned_Y,pinned_Z,nn);

%% Group/Clustered information 
%generate group index
% gr=[];
gr_bar=[1:7:7*(p-1)+1];     %  bars
gr_ridge=[kron(ones(1,p),[2,3])+7*kron(0:p-1,[1,1])];      % ridge strings
gr_diagonal=[kron(ones(1,p),[4,5])+7*kron(0:p-1,[1,1])];      % diagonal strings
gr_bottom=[6:7:7*(p-1)+6];     % bottom strings
gr_top=[7:7:7*p];     % bottom strings
gr={gr_bar;gr_ridge;gr_diagonal;gr_bottom;gr_top};     % group ridge strings,diagonal strings, bottom strings, top strins

% several clustered string in one group of ridge and diagonal strings
num_clu=4;
gr_ridge2=reshape(gr_ridge,numel(gr_ridge)/num_clu,[])';
gr_diagonal2=reshape(gr_diagonal,numel(gr_diagonal)/num_clu,[])';
gr2=mat2cell([gr_ridge2;gr_diagonal2],ones(1,2*num_clu));
gr2=[gr2;gr_bottom;gr_top]
% gr2=[gr2]
...
%% plot member force 
tenseg_plot_result(1:substep,t_t([1:3,2*p+1:2*p+3],:),{'1','2','3','4','5','6'},{'Load step','Force (N)'},'plot_member_force.png',saveimg);
