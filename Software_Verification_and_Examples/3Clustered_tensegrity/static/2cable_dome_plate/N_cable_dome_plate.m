function [N,C_b,C_s,C] =N_cable_dome_plate(R,rate,p,m,h,beta)
...
%     N0(:,i)=[i*l/2/(m+1)-R;0;sqrt(Rd^2-((m+1-i)*l/2/(m+1))^2)-(Rd-h)];
    N0(:,i)=[(1-rate)*i/m*R-R;0;sqrt(Rd^2-((m+1-i)*l/2/(m+1))^2)-(Rd-h)];
end
...
% N0(1,[2,4])=-R*rate*ones(1,m);
% N0(1,[1,3])=-R*(0.5+0.5*rate)*ones(1,m);
