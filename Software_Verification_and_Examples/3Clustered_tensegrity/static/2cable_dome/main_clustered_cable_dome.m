...
Ke_aa=A_1ag*diag(E_gp.*A_gp./(l_c.^3))*A_1ag';
Kt_aa=Kg_aa+(Ke_aa+Ke_aa')/2;       % this is to 
[V_mode,D1] = eig(Kt_aa);         % eigenvalue of tangent stiffness matrix
d1=diag(D1);   
%% plot the mode
figure
plot(1:size(Ia,2),d1,'k-o','linewidth',1.5);
set(gca,'fontsize',18);
xlabel('Order of Vibration Mode','fontsize',18,'Interpreter','latex');
ylabel('Frequency (Hz)','fontsize',18,'Interpreter','latex');
% grid on;
if saveimg==1
    saveas(gcf,'eigenvalue of tangent stiffness matrix.png');
end
num_plt=1:6
ampli=0.1
for i=1:numel(num_plt)
    f1=figure;
    title=({['mode ',num2str(num_plt(i))];['f=',num2str(d1(num_plt(i)),'%.4f'),'Hz']});
    %plot buckling mode
...
%% mode analysis
num_plt=1:4;        % number of modes to plot
[V_mode,omega]=tenseg_mode(Ia,C,C_b,C_s,N(:),E,A,l0,M,num_plt,saveimg,0.1,S);
