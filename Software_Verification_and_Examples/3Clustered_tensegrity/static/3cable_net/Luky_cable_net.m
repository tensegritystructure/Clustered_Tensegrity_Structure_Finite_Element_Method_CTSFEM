...
R=25;          %radius
p=7;          %complexity for cable dome(Outer ring node)
% rate1=19.319/50;
 rate1=0.625;

% generate node in one unit
beta1=pi/(p);beta2=pi/(8*p);beta3=pi/(1.6*p);
...
N_2_0=R*[T3*[1;0;0]];      %外节点
N_1_0=R*[rate1*T2*[1;0;0]];       %内节点
N_2=[];
N_1=[];
...
% C_s_in=[[2:3:3*p-1]',[3:3:3*p]';[2:3:3*p-1]',[6:3:3*p,3]';[3:3:3*p]',[6:3:3*p,3]'];
%C_s_in=[[1:2:2*p-10]',[2:2:2*p-10]';[2:2:2*p-12 24]',[3:2:2*p-11 1]';[2:2:2*p]',[4:2:2*p 2]']

C_s_in=[[1:1:p]',[p+2:1:2*p+1]';[1:1:p]',[p+1:1:2*p]';[p+1:1:2*p]',[p+2:1:2*p+1]'];
...
pinned_X=([1:1:p,p+1,2*p+1])'; pinned_Y=([1:1:p,p+1,2*p+1])'; pinned_Z=([1:1:p,p+1,2*p+1])';
[Ia,Ib,a,b]=tenseg_boundary(pinned_X,pinned_Y,pinned_Z,nn);
%% %% Group/Clustered information 
%generate group index
% gr=[];
%gr={[1:p,3*p+1:4*p]',[4*p+1:5*p]',[5*p+1:6*p]'}
gr={[1:p,p+1:2*p]';[2*p+1:3*p]'};  % outer diagonal, inner diagonal, inner hoop
...
A_N_1=0.6320*1e-3;    %内环截面面积    >0.3320*1e-2
A_N_2=0.3280*1e-3;     %外环截面面积   >0.2280*1e-2
 %A_s=[A_N_2;A_N_1];
[A_b,A_s,A_c,A,r_b,r_s,r_gp,radius,E_c,l0_c,rho,mass_c]=tenseg_minimass_1(A_N_1,A_N_2,p,t_c,l_c,eye(size(S,1)),sigmas,sigmab,Eb,Es,index_b,index_s,c_b,c_s,rho_b,rho_s,thick,hollow_solid);
E=S'*E_c;     %Young's modulus CTS
%A_s=[A_N_2;A_N_1];       %弦成员的截面积
%A_c=[A_N_2;A_N_1];       %组内成员的截面积
...
%% plot member force 
tenseg_plot_result(1:substep,t_t([p,2*p],:),{'1','2'},{'Load step','Force (N)'},'plot_member_force.png',saveimg);
