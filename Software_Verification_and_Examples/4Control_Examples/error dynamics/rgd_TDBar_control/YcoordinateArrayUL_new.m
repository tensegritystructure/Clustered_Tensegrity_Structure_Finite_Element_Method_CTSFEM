function y_p = YcoordinateArrayUL_new(iaf,x_p)
%
...
%     %% Update the x_p into upper and lower separately! Important for Asymmetric Wing
%     for i=1:length(x)
%         if x(i)==x_p      
%         Insert_idx=i;
%         end
...
x_pu=[];x_pl=[];
for i = 1:length(idx)
    x_pu(i)=x_p(i)-yt(idx(i))*sin(theta(idx(i)));
    %x_pu1=[x_pu1 x_pu(i)];
    x_pl(i)=x_p(i)+yt(idx(i))*sin(theta(idx(i)));
    %x_pl1=[x_pl1 x_pu(i)];
...
y_p=[];  
for j=1:length(x_p)
    idx_p=find(abs(af.x-x_p(j))==0);
    y_pp=unique(af.z(idx_p));
    y_p=[y_p;y_pp];
end
