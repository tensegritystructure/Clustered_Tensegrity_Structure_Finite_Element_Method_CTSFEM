function [fig_out] = tenseg_plot_target_3D_mov( N,C_b,C_s,target,fig_handle,highlight_nodes,view_vec )
...
if ~isempty(C_b),
    ...
    for j = 1:size(B,2),
    	...
    	quiver3(bar_start_nodes(1,:),bar_start_nodes(2,:),bar_start_nodes(3,:),B(1,:),B(2,:),B(3,:),'black.','Autoscale','off','LineWidth',3,'ShowArrowHead','off')
...
if ~isempty(C_s),
    ...
    for j = 1:size(S,2),
    	...
    	quiver3(string_start_nodes(1,:),string_start_nodes(2,:),string_start_nodes(3,:),S(1,:),S(2,:),S(3,:),'red.','Autoscale','off','LineWidth',2,'ShowArrowHead','off');
...
for j=1:numel(highlight_nodes),
    ...
    plot3(N(1,node_index),N(2,node_index),N(3,node_index),'bd','MarkerSize',8,'MarkerFaceColor','blue')
end
plot3(target_new(:,1),target_new(:,2),target_new(:,3),'bo','MarkerSize',8,'MarkerFaceColor','blue')
% Modify plot display
grid off; axis equal;
if isempty(view_vec),
