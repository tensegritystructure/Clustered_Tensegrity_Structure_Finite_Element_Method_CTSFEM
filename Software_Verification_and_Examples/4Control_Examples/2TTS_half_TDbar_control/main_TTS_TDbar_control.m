...
ind_n_ct=[8];n_ct1=[0.1];n_ct2=[0.1];
% ind_n_ct=[7];n_ct1=[1.9];n_ct2=[1.9];
...
%% strings and bars as active members
%% choose active, passive members
ind_act=[5:8]; ind_pas=setdiff(1:ne,ind_act);
% ind_act=[7:8]; ind_pas=[1:6];
% ind_act=[6];  ind_pas=setdiff(1:ne,ind_act);
...
% % this is a wrong way
% Control_index1=TAU_m'*M_check        %row is active member,column is target acceleration
...
I_bar=transfer_matrix(index_b,1:size(S,1));
I_str=transfer_matrix(index_s,1:size(S,1));
% TAU_b TAU_s for bars and strins
TAU_b=TAU*I_bar   
TAU_s=TAU*I_str
TAU_1=[TAU_s,kron([1,-1],TAU_b)] 

rref(TAU_1)

[R,p]=rref(TAU_1)
for i=1:size(TAU_1,1)
R_num{i,1}=find(R(i,:)<0);
end
