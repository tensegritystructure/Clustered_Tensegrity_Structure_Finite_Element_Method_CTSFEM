...
function [N,CBT,CST] = Wing2_Connectivity(iaf,q,x_coord,RGD,H,Jy_t)
...
%% Node matrix
for i=1:q+2
    N(:,i)=[x_coord(i);Jy_t(i);0];
end

for i=q+3:2*q+2
    N(:,i)=[x_coord(i-q-1);y_coord(i-q-2)+Jy_t(i-q-1);0];
end

for i=2*q+3:3*q+2
    N(:,i)=[x_coord(i-2*q-1);-H(i-2*q-1)+y_coord(i-2*q-2)+Jy_t(i-2*q-1);0];
end
% H=[];
% for i = q+3:2*q+2
%     H=[H 2*y_coord(i-q-2)];
