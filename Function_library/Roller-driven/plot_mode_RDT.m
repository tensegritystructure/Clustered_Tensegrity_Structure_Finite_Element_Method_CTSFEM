function plot_mode_RDT(Mode,value,N,R,Ia,C,index_b,S,l,title,xlb,ylb,unit,num_plt,ampli,saveimg,view_vec)
...
%     title2=(['mode ',num2str(num_plt(i)),': ',num2str(value(num_plt(i)),'%.2e'),unit]);
    %plot buckling mode
    tenseg_plot_RDT(N+ampli*max(l)*reshape(Ia*Mode(:,num_plt(i))/norm(Mode(:,num_plt(i))),3,[]),C,R,index_b,S,f1,[],[],title2,[],[],[],[]);
    tenseg_plot_RDT_dash(N,C,R,index_b,S,f1,[],[],title2,[],[],[],[]);
     axis off;
        view(view_vec);
    if saveimg==1
