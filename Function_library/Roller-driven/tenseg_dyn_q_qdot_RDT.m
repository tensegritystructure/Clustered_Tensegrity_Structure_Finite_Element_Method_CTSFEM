function [Yd,f,l,q]=tenseg_dyn_q_qdot_RDT(t,Y,data_in)
% Input:
%   t: current time
...
Kn=kron(C'*diag(f./l)*C,eye(3));                      %stiffness matrix
...
qa_dd=(E_qa'*M*E_qa)\(E_qa'*(w-M*E_qb*qb_dd-[Kn*n;C'*f]-D*q_d));      %dynamic equation
Yd=[qa_d;qa_dd];
