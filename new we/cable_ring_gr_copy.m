...
%         gr={[2*p+1:4*p]';[1:2*p]';[4*p+1:5*p]'};  % 上，下，环；一组
        ...
        gr_1={[1:2*p]';[2*p+1:4*p]';[4*p+1:6*p]';[6*p+1:7*p]';[7*p+1:9*p]';[9*p+1:11*p]'};  % 上，下，环；一组 ；索矩阵
...
%         gr={[4*p+1:5*p]';[2*p+1:4*p]';[1:2*p]'};  % 上，下，环；一组
    case 2
        ...
        gr_hs1= [6*p+1:13*p/2]';
        gr_hs2=[13*p/2+1:7*p]';
        gr_1={gr_hs1;gr_hs2;gr_ssxs1;gr_ssxs2;gr_sxs1;gr_sxs2;gr_xxs1;gr_xxs2;gr_zsxs1;gr_zsxs2;gr_zxxs1;gr_zxxs2}; %两组; 环，上，下
        gr = [gr_1;Gr];
    ...
    case 3
        ...
        gr_hs3=[20*p/3+1:7*p]';
        gr_1={gr_hs1;gr_hs2;gr_hs3;gr_xxs1;gr_xxs2;gr_xxs3;gr_ssxs1;gr_ssxs2;gr_ssxs3;gr_sxs1;gr_sxs2;gr_sxs3;gr_zsxs1;gr_zsxs2;gr_zsxs3;gr_zxxs1;gr_zxxs2;gr_zxxs3}; %三组;环，上, 下，
        gr = [gr_1;Gr];
    ...
    case 6
        ...
        gr_xxs1=[[kron(ones(1,p/6),[1,2])+2*kron(0:(p-1)/6,[1,1])+gr_fz]';[kron(ones(1,p/6),[2*p+1,2*p+2])+2*kron(0:(p-1)/6,[1,1])+gr_fz+[zeros(1,p/6),ones(1,p/6)]]'];
        gr_xxs2=[[kron(ones(1,p/6),[p/6+1,p/6+2])+2*kron(0:(p-1)/6,[1,1])+gr_fz]';[kron(ones(1,p/6),[13*p/6+1,13*p/6+2])+2*kron(0:(p-1)/6,[1,1])+gr_fz+[zeros(1,p/6),ones(1,p/6)]]'];
        gr_xxs3=[[kron(ones(1,p/6),[2*p/6+1,2*p/6+2])+2*kron(0:(p-1)/6,[1,1])+gr_fz]';[kron(ones(1,p/6),[14*p/6+1,14*p/6+2])+2*kron(0:(p-1)/6,[1,1])+gr_fz+[zeros(1,p/6),ones(1,p/6)]]'];
        gr_xxs4=[[kron(ones(1,p/6),[3*p/6+1,3*p/6+2])+2*kron(0:(p-1)/6,[1,1])+gr_fz]';[kron(ones(1,p/6),[15*p/6+1,15*p/6+2])+2*kron(0:(p-1)/6,[1,1])+gr_fz+[zeros(1,p/6),ones(1,p/6)]]'];
        gr_xxs5=[[kron(ones(1,p/6),[4*p/6+1,4*p/6+2])+2*kron(0:(p-1)/6,[1,1])+gr_fz]';[kron(ones(1,p/6),[16*p/6+1,16*p/6+2])+2*kron(0:(p-1)/6,[1,1])+gr_fz+[zeros(1,p/6),ones(1,p/6)]]'];
        gr_xxs6=[[kron(ones(1,p/6),[5*p/6+1,5*p/6+2])+2*kron(0:(p-1)/6,[1,1])+gr_fz]';[kron(ones(1,p/6),[17*p/6+1,17*p/6+2])+2*kron(0:(p-1)/6,[1,1])+gr_fz+[zeros(1,p/6),ones(1,p/6)]+[zeros(1,2*p/6-1),-p]]'];
...
%          matrix2 = [[1:1:2*p]',[2*p+1:1:4*p]']';  %生成 p*1 的矩阵，上斜索
         ...
         matrix5 = [7*p+1:1:9*p]';  %生成 p*2 的矩阵，主上斜索
         matrix6 = [9*p+1:1:11*p]';  %生成 p*2 的矩阵，主下斜索
         matrix = [matrix3;matrix2;matrix1;matrix4;matrix5;matrix6];
