...
xp=1;                %细分开合度用
 rate0=linspace(0.0001,0.0001,xp);
p=6;          %complexity for cable dome(Outer ring node)；2的倍数
n_t0=zeros(3*9*p,xp);%7*p为点数
...
gbj=(2*pi-pi*(p-2)/p-pi/3)/2; %小三角杆旁边的角的角度
    ...
    N_8_0=[T2*[(sgg/cos(pi/6))*(R*(cos(pi/p))^2-r0)/((h2-h4)^2+(R*(cos(pi/p))^2-r0)^2)^0.5+r0;0;h5]];   %环索主力上斜索杆
    N_9_0=[T2*[(sgg/cos(pi/6))*(R*(cos(pi/p))^2-r0)/((h3-h4)^2+(R*(cos(pi/p))^2-r0)^2)^0.5+r0;0;h6]];   %环索主力下斜索杆
    ...
    N=[N_3,N_4,N_1,N_2,N_5,N_6,N_7,N_8,N_9];  

    C_b_in=[[4*p+1:1:5*p]',[7*p+1:1:8*p]';[4*p+1:1:5*p]',[8*p+1:1:9*p]'];

    C_s_in=[[1:1:p]',[4*p+1:1:5*p]';[p+1:1:2*p]',[4*p+1:1:5*p]';[4*p+1:1:5*p]',[2*p+1:1:3*p]';[4*p+1:1:5*p]',[2*p+2:1:3*p,2*p+1]';[2*p+1:1:3*p]',[3*p+1:1:4*p]';
        [2*p+2:1:3*p,2*p+1]',[3*p+1:1:4*p]';[4*p+1:1:5*p]',[4*p+2:1:5*p,4*p+1]';[5*p+1:1:6*p]',[7*p+1:1:8*p]';
        [5*p+2:1:6*p,5*p+1]',[7*p+1:1:8*p]';[6*p+1:1:7*p]',[8*p+1:1:9*p]';[6*p+2:1:7*p,6*p+1]',[8*p+1:1:9*p]'];
...
[gr] = cable_ring_gr_copy_experiment3(gr_num,p);
...
[U1,U2,V1,V2,S1]=tenseg_svd(A_2ag);%U1列空间；V1行空间；U2左零空间；V2零空间；S特征值，(2.49)
...
for jj=1:xp
    q=q_xp(:,jj);
A_1az=A_1axp(:,jj);
A_1a=reshape(A_1az,[],78);
l_c=l_cxp(:,jj);
[Kt_aa,Kg_aa,Ke_aa,K_mode,k]=tenseg_stiff_CTS3(Ia,C,S,t_c,A_2a,E_c,A_c_row,l0,l);
