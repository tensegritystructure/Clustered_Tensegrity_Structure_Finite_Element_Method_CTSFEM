...
l0_cxp=zeros(7*p,xp);%10*p为索和杆的总数
t_cxp=zeros(7*p,xp);
t_xp=zeros(10*p,xp);
A_xp=zeros(10*p,xp);
l_xp=zeros(10*p,xp);
...
gbj=(2*pi-pi*(p-2)/p-pi/3)/2; %小三角杆旁边的角的角度
 ...
 N_7=[];
    ...
    C_s_in=[[1:1:p]',[5*p+1:1:6*p]';[p+1:1:2*p]',[6*p+1:1:7*p]';[5*p+1:1:6*p]',[2*p+2:1:3*p,2*p+1]';[6*p+1:1:7*p]',[2*p+1:1:3*p]';
            [2*p+1:1:3*p]',[3*p+1:1:4*p]';[2*p+2:1:3*p,2*p+1]',[3*p+1:1:4*p]';[4*p+1:1:5*p]',[4*p+2:1:5*p,4*p+1]';[1:1:p]',[p+1:1:2*p]'];
...
pinned_X=([1:1:2*p,3*p+1:1:4*p])'; pinned_Y=([1:1:2*p,3*p+1:1:4*p])'; pinned_Z=([1:1:2*p,3*p+1:1:4*p])';
...
[gr] = cable_ring_gr_copy_experiment2(gr_num,p);
